...
class TelaInicial(QMainWindow):
    def __init__(self):
        ...
        for i, v in enumerate(self.db.lista_contas):
            self.cont += 1
    ...
    def cadastro(self):
        if self.ui.input_nome_2.text().isnumeric() or self.ui.input_sobrenome_2.text().isnumeric() or len(
            ...
            try:
                ...
                for v in db.lista_contas:
                    nome_completo_na_bd = f'{str(v["nome"]).capitalize()} {str(v["sobrenome"]).capitalize()}'
                    nome_completo_nos_inputs = f'{str(self.ui.input_nome_2.text()).capitalize()} {str(self.ui.input_sobrenome_2.text()).capitalize()}'
                    if nome_completo_na_bd == nome_completo_nos_inputs:
                        assert ()

                nome_da_tabela = f'{str(self.ui.input_nome_2.text())}{str(self.ui.input_sobrenome_2.text()).capitalize()}'
                db.tabela_cada_conta(nome_da_tabela)
                db.inserir_contas(self.ui.input_nome_2.text(), self.ui.input_sobrenome_2.text())
